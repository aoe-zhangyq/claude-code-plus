...
3. 设置消息：`SetCommitMessage(message="feat: 添加功能", mode="replace")`
    ...
    /**
     * SVN MCP 工具 Schema（JSON 格式）
     */
    val SVN_TOOLS_SCHEMA = """
{
  "GetVcsChanges": {
    "type": "object",
    "description": "Get uncommitted VCS changes in the current project. Returns file paths, change types, and optionally diff content.",
  ...
  "GetSvnHistory": {
    "type": "object",
    "description": "Get SVN commit history. Returns author, date, message, and changed paths for each commit.",
    "properties": {
      "maxCount": {
        "type": "integer",
      ...
      "revision": {
        "type": "string",
        "description": "Specific revision number to get info for (e.g., '12345'). If provided, returns only that revision."
      },
      "filePath": {
        "type": "string",
        "description": "Specific file path to get history for. If provided, returns history for that file only."
      }
    },
    "required": []
  },
  ...
  "GetSvnStatus": {
    "type": "object",
    "description": "Get SVN status overview: current URL, branch/tag info, revision, number of changes, etc.",
    "properties": {},
    "required": []
  }
}
     ...
     * SVN MCP 默认提示词（英文）
     */
    private val SVN_INSTRUCTIONS_EN = """
### SVN MCP

Tools for interacting with IDEA's VCS/SVN integration:

- `mcp__jetbrains_svn__GetVcsChanges`: Get uncommitted changes (supports selectedOnly for Commit panel selection)
- `mcp__jetbrains_svn__GetSvnHistory`: Get SVN commit history (supports revision, filePath, maxCount filters)
- `mcp__jetbrains_svn__GetCommitMessage`: Get current commit message from input field
- `mcp__jetbrains_svn__SetCommitMessage`: Set or append commit message
- `mcp__jetbrains_svn__GetSvnStatus`: Get SVN status (URL, branch/tag, revision, changes count, etc.)

**Usage:**
1. Get changes: `GetVcsChanges(selectedOnly=true, includeDiff=true)`
2. Get history: `GetSvnHistory(maxCount=10)` or `GetSvnHistory(revision="12345")` or `GetSvnHistory(filePath="src/main.kt", maxCount=20)`
3. Read message: `GetCommitMessage()`
4. Set message: `SetCommitMessage(message="feat: add feature", mode="replace")`

**SVN Commit Message Convention:**
When generating commit messages for SVN:
- Keep it concise but descriptive
- Include file/function context when relevant
- For bug fixes: mention the issue number if available
- For features: briefly describe what and why
    """.trimIndent()

    /**
     * SVN MCP 默认提示词（中文）
     */
    private val SVN_INSTRUCTIONS_ZH = """
### SVN MCP

与 IDEA 的 VCS/SVN 集成交互的工具：

- `mcp__jetbrains_svn__GetVcsChanges`：获取未提交的更改（支持 selectedOnly 以获取提交面板的选择）
- `mcp__jetbrains_svn__GetSvnHistory`：获取 SVN 提交历史（支持 revision、filePath、maxCount 过滤器）
- `mcp__jetbrains_svn__GetCommitMessage`：从输入框获取当前提交消息
- `mcp__jetbrains_svn__SetCommitMessage`：设置或追加提交消息
- `mcp__jetbrains_svn__GetSvnStatus`：获取 SVN 状态（URL、分支/标签、修订版、更改计数等）

**使用方法：**
1. 获取更改：`GetVcsChanges(selectedOnly=true, includeDiff=true)`
2. 获取历史：`GetSvnHistory(maxCount=10)` 或 `GetSvnHistory(revision="12345")` 或 `GetSvnHistory(filePath="src/main.kt", maxCount=20)`
3. 读取消息：`GetCommitMessage()`
4. 设置消息：`SetCommitMessage(message="feat: 添加功能", mode="replace")`

**SVN 提交消息规范：**
生成 SVN 提交消息时：
- 保持简洁但描述清楚
- 相关时包含文件/函数上下文
- 对于 bug 修复：提及问题编号（如果有）
- 对于新功能：简要说明内容和原因
    """.trimIndent()
     ...
     * 获取 SVN MCP 默认提示词
     */
    fun getSvnInstructions(language: String): String {
        return when (language) {
            "zh" -> SVN_INSTRUCTIONS_ZH
            else -> SVN_INSTRUCTIONS_EN
        }
    }

    // 向后兼容：保留旧的常量
    val SVN_INSTRUCTIONS = SVN_INSTRUCTIONS_EN
}
...
object KnownTools {
     ...
     */
    val SVN_MCP = listOf(
        "mcp__jetbrains_svn__GetVcsChanges",    // 获取变更
        "mcp__jetbrains_svn__GetSvnHistory",    // 获取 SVN 历史记录
        "mcp__jetbrains_svn__GetCommitMessage", // 获取 commit message
        "mcp__jetbrains_svn__SetCommitMessage", // 设置 commit message
        "mcp__jetbrains_svn__GetSvnStatus"      // 获取 SVN 状态
    )

    /**
     * 所有已知工具
     */
    val ALL = CLAUDE_BUILT_IN + JETBRAINS_MCP + TERMINAL_MCP + GIT_MCP + SVN_MCP
}
